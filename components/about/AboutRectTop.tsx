...
export const AboutRectTop: FunctionComponent = () => {
  const DEFAULT_TOP_TOP: string = '-50%'
  const DEFAULT_TOP_LEFT: string = '-65%'
  ...
  return (
    <>
      <motion.div initial="hidden" animate="visible" variants={topRectangleVariants} className="absolute w-screen h-screen bg-gradient-to-br from-red-500 to-yellow-500 opacity-25" />
      <motion.div initial="hidden" animate="visible" variants={bottomRectangleVariants} className="absolute w-screen h-screen bg-gradient-to-br from-blue-400 to-purple-400 opacity-25" />
    </>
  )
}